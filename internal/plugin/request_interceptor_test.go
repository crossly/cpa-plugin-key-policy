...
func TestQuotaFrontendAuthFailsClosedOutsideOpenAIPaths(t *testing.T) {
	app, plain, now := configureQuotaResponseTestApp(t, 10, 50)
	app.Store().SetClock(func() time.Time { return now })
	app.Store().RecordUsage("quota-key", "fast", "gpt-5-codex", false, policy.UsageDetail{
		InputTokens: 10_000_000,
	...
	auth := frontendAuthForTest(t, app, "/v1/messages", http.Header{"Authorization": {"Bearer " + plain}})
	if auth.Authenticated {
		t.Fatalf("non-OpenAI quota auth = %+v, want rejected", auth)
	}
}
...
func TestIsOpenAIQuotaPathAcceptsCPAPathPrefixes(t *testing.T) {
	for _, path := range []string{
		...
		if !isOpenAIQuotaPath(path) {
			t.Errorf("isOpenAIQuotaPath(%q) = false, want true", path)
		}
	}
	for _, path := range []string{"/v1/messages", "/v1/models", "/v1/usage", "/v1beta/models/gemini:generateContent"} {
		if isOpenAIQuotaPath(path) {
			t.Errorf("isOpenAIQuotaPath(%q) = true, want false", path)
		}
	}
}

func configureQuotaResponseTestApp(t *testing.T, daily, weekly float64) (*App, string, time.Time) {
	t.Helper()
	app := NewApp()
...
`, filepath.ToSlash(filepath.Join(t.TempDir(), "state.json")), hash, daily, weekly))
	req, _ := json.Marshal(LifecycleRequest{ConfigYAML: yaml, SchemaVersion: SchemaVersion})
